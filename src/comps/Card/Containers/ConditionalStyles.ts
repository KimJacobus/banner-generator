...
const flexStyles = ({ $cardData }: cardPropsT) => {
   const returnStyles = (type: flexStrings): Interpolation<cardPropsT> => {
      ...
      return css`
         ...
         align-items: ${defPath?.alignItems};
         flex-direction: ${defPath?.flexDirection};
         margin: ${defPath?.margin};
         gap: ${defPath?.gap};
         width: ${defPath?.width};
         height: ${defPath?.height};
         padding: ${defPath?.padding};
         flex-wrap: ${defPath?.flexWrap};
         overflow: ${defPath?.overflow || 'hidden'};
         border-radius: ${defPath?.borderRadius || '2px'};
